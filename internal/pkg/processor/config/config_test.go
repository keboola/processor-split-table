...
func TestConfig(t *testing.T) {
	...
	for _, testData := range getTestData() {
		...
		if testData.expected != nil {
			require.NoError(t, err, testData.comment)
	...
	return []testData{
		{
			...
			error:    `invalid configuration: key "parameters" has invalid type "string", expected "object"`,
			expected: nil,
		},
		{
		...
		},
		{
			...
			expected: &Config{
				Parameters: slicerConfig.Config{
					...
					LogInterval: slicerConfig.LogIntervalConfig{
						Multiplier: 1.5,
		...
		{
			comment: "mode-slices-min-size-string",
			input:   "{\"parameters\": {\"inputSizeThreshold\": \"10MB\"}}",
			error:   "",
			expected: &Config{
				Parameters: slicerConfig.Config{
					Mode:             slicerConfig.ModeBytes,
					...
					AheadBlocks:        16,
					AheadBlockSize:     datasize.MB,
					InputSizeThreshold: 10 * datasize.MB,
					Gzip:               true,
					GzipLevel:          1,
					GzipConcurrency:    0,
					GzipBlockSize:      1 * datasize.MB,
